...
--			if RaidRoll_DB["debug"] == true then RR_Test("New Mob Found!") end
	...
	--						if RaidRoll_DB["Loot"][Max_LootID] == nil then RaidRoll_DB["Loot"][Max_LootID] = mob_guid end
						...
						-- Create an array
							...
							_, _, _, ItemLvl = GetItemInfo(ItemId);
						...
						-- Special rare items that should be included
							if 	ItemId == 	46110 	or		-- Alchemist's Cache
								...
								ItemId == 	49908	then	-- Primordial Saronite
									AcceptItem = true
									if RaidRoll_DB["debug"] == true then RR_Test("This is an acceptable Item - " .. lootName) end
							else
								if RaidRoll_DB["debug"] == true then RR_Test("This is >NOT< an acceptable Item - " .. lootName) end
							end
...
function RR_MouseOverTooltip(ID)
	if RR_Tooltips == nil then 
		...
		RR_Tooltips["RR_Loot_LinkLootButton"]="Link the boss loots to the raid"
		RR_Tooltips["RaidRoll_AnnounceWinnerButton"]="Announce the winner"
		RR_Tooltips["RR_Roll_RollButton"]="Perform a 1-100 roll"
	...
	--Check if the player rolling is the user
		if Name == UnitName("player") then
			...
			if RR_Name_Array[Roll] ~= nil and RR_ItemLink == nil then
				SendChatMessage(">>> " .. RR_Name_Array[Roll] .. " Wins! <<<", rr_AnnounceType1)
			end
			
			if RR_Name_Array[Roll] ~= nil and RR_ItemLink ~= nil then
				SendChatMessage(">>> " .. RR_Name_Array[Roll] .. " Wins " .. RR_ItemLink .. "! <<<", rr_AnnounceType1)
			end
...
-- Function donated by zanglang
-- Originally used in RollTracker Lite
-- URL: http://wow.curse.com/downloads/wow-addons/details/rolltracker.aspx
...
local Name,Roll,Low,High
	...
	-- Killerpet würfelt. Ergebnis: 1 (1-100)
	...
	if msg ~= nil then 
		if string.find(msg, "%(") ~= nil then
			if string.find(msg, "%)") ~= nil then
				...
				--	musou,     _,   99        ,    (1-100)
					name, _, roll, lowhigh, pad1, pad2 = strsplit(" ",msg, 6)
				...
				-- Make sure we have no nil values
					if name ~= nil and roll~= nil and lowhigh~= nil then
					...
					-- Fylox tira los dados y obtiene 8 (1-100) 
						
					-- just in case (x10), hunts down the roll and the max/min rolls
						pad3 = ""
						for i=1,10 do
							if tonumber(roll) == nil then  
								if pad2 ~= nil then pad2,pad3 = strsplit(" ",pad2,2) end
								
								if RaidRoll_DB["debug"] == true then 
									RR_Test("*****Roll Message Splitting*****")
									RR_Test(tonumber(roll))
									RR_Test(lowhigh)
									RR_Test(pad1)
									RR_Test(pad2)
									RR_Test(pad3)
					...
					-- Make sure we have no nil values (again)
						if name ~= nil and roll~= nil and lowhigh ~= nil then
							
						-- (1-100)
							lowhigh = string.sub(lowhigh, 2, strlen(lowhigh)-1)
							
							low, high = strsplit("-",lowhigh)
...
--if rr_rollID == nil then rr_rollID = rr_CurrentRollID end 
	...
	-- Setting the players name, roll, guild rank and roll status to the current max players value
		if rr_AddedPlayer == true then
			...
			PR,AboveThreshold,EP,GP = RR_GetEPGPCharacterData(PlayerName)
...
-- This allows auto-reporting of 10 and 5 seconds left
	...
	if 	rr_Item[rr_CurrentRollID] ~= "ID #"..rr_CurrentRollID and (time() > RR_Timestamp + 59 or rr_CurrentRollID ~= rr_rollID) then
		...
		Winner = RR_FindWinner(rr_CurrentRollID)
		...
		if time() > RR_Timestamp + 60 then
			if RR_RollCountdown == true then
				if RaidRoll_DBPC[UnitName("player")]["RR_RollCheckBox_Auto_Announce"] == true or RR_AnnounceCountdowns == true then
					...
					Winner,Roll,EPGP = RR_FindWinner(rr_rollID)
					
					if Winner  ~= "" then
						if RaidRoll_DBPC[UnitName("player")]["RR_EPGP_Enabled"] ==  true then
							if rr_Item[rr_rollID] == "ID #"..rr_rollID then
								SendChatMessage(string.format(RAIDROLL_LOCALE["won_PR_value"], Winner,EPGP), rr_AnnounceType)
							else
								SendChatMessage(string.format(RAIDROLL_LOCALE["won_item_PR_value"], Winner, rr_Item[rr_rollID],EPGP), rr_AnnounceType)
							end
						else
							if rr_Item[rr_rollID] == "ID #"..rr_rollID then
								SendChatMessage(string.format(RAIDROLL_LOCALE["won_with"], Winner,Roll), rr_AnnounceType)
							else
								SendChatMessage(string.format(RAIDROLL_LOCALE["won_item_with"], Winner,rr_Item[rr_rollID],Roll), rr_AnnounceType)
							end
						end
					else
						SendChatMessage(string.format(RAIDROLL_LOCALE["No_winner_for"], rr_Item[rr_rollID]), rr_AnnounceType)
					end
				end
...
cmd_s = string.lower(cmd)
	...
	if cmd_s == "zone" then 
		...
		if RR_NameAmount > 0 then
			...
			if RR_DifferentZone ~= "" then
				SendChatMessage("Players in another zone: " .. RR_DifferentZone, "RAID")
			end
			
			if RR_OutOfRange ~= "" then
				SendChatMessage("Players >28 yd from me: " .. RR_OutOfRange, "RAID")
			end
			
			if RR_DifferentZone == "" and RR_OutOfRange == "" then
				SendChatMessage("Everyone is here. Group hug! /cheer", "RAID")
			end
		end
...
-- Output the list of people to raid chat if its not a reroll
	...
	-- Check if there is an item being rolled on
		if ItemLink == nil then
			SendChatMessage("<<Raid Rolling>> (ID - Name)" , rr_AnnounceType2)
		else
			SendChatMessage("<<Raid Rolling for: " .. ItemLink .. " >> (ID - Name)" , rr_AnnounceType2)
		end
	...
	-- Check if there is an item being rolled on
		if ItemLink == nil then
			SendChatMessage("<<Re-Rolling>>" , rr_AnnounceType2)
		else
			SendChatMessage("<<Re-Rolling for: " .. ItemLink .. " >>" , rr_AnnounceType2)
		end
	end
