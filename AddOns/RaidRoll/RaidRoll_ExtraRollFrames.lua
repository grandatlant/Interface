...
-- FRAME NOT LOADED MESSAGE
 ...
 RR_RollFrame:SetScript("OnShow", RR_GuildRankUpdate)
 RR_RollFrame:SetScript("OnShow", RR_GetEPGPGuildData)
...
-- Create 5 buttons to mark up names
	for i=1,5 do
		...
		Raid_Roll_SetSymbol:SetNormalTexture(nil)
		Raid_Roll_SetSymbol:SetPushedTexture(nil)
...
-- RaidRoll_AnnounceWinnerButton
	...
	RaidRoll_AnnounceWinnerButton:SetScript("OnClick",	function()
															Winner,Roll,EPGP = RR_FindWinner(rr_CurrentRollID)
...
--[[
	...
	StaticPopupDialogs["Clear_Rolls"] = {
		text = "Clear all saved roll memory?",
		button1 = "Yes",
		button2 = "No",
		OnAccept = function()
						rr_rollID = 0
						...
						RR_Display(rr_CurrentRollID)
						RR_Test("All saved rolls cleared")
					end,
		timeout = 0,
		whileDead = true,
		hideOnEscape = true,
...
if RaidRoll_DB["debug"] == true then RR_Test("---- RR_FinishRolling ----" ) end
	...
	if dontannounce ~= true then
		...
		if Winner  ~= "" and rr_Item[rr_CurrentRollID] ~= "ID #"..rr_CurrentRollID and (time() > RR_Timestamp + 60 or rr_CurrentRollID ~= rr_rollID) then
			if RaidRoll_DB["debug"] == true then RR_Test("---- Awarding Loot ----" ) end
		-- Get the number of items on the body
			...
			-- If its an item
				if LootSlotIsItem(i) then
					if WeHaveFoundTheItem == false then
						...
						if ItemLink == rr_Item[rr_CurrentRollID] then
			...
			if WeHaveFoundTheItem == false then
				RR_Test(rr_Item[rr_CurrentRollID] .. " was not found. Please ensure you have the loot window open.")
			end
		end
	end
end 
...
function RR_ReallyGiveLoot(player,slot)
	...
	if RaidRoll_DB["debug"] == true then RR_Test("Giving " .. player .. " the loot in slot " .. slot) end
