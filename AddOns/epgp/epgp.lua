...
local function DecayEPGPAsync()
  local decay = global_config.decay_p  * 0.01
  local reason = string.format("Decay %d%%", global_config.decay_p)

  SendChatMessage("EPGP: Уменьшение EP/GP Этап 1 Асинхронизация", 'GUILD', nil, 0 );
  local i=1;
  for name,_ in pairs(ep_data) do
	if i % 50 == 0 then
		SendChatMessage("EPGP: Уменьшение EP/GP Этап "..(i/50+1), 'GUILD', nil, 0 );
		Coroutine:Sleep(0.5)
	end
    local ep, gp, main = self:GetEPGP(name)
    assert(main == nil, "Corrupt alt data!")
...
function delay_s(delay)
   delay = delay or 1
   local time_to = GetTime() + delay
   while GetTime() < time_to do end
end
...
function EPGP:DecayEPGP()
  ...
  for name,_ in pairs(ep_data) do
	if i % 30 == 0 then
		SendChatMessage("EPGP: Уменьшение EP/GP Этап "..(i/30+1), 'GUILD', nil, 0 );
		delay_s(100)
	end
    local ep, gp, main = self:GetEPGP(name)
    assert(main == nil, "Corrupt alt data!")
...
function EPGP:IncEPGPBy(name, reason, amountEp, amountGp, mass, undo)
	assert(EPGP:CanIncGPBy(reason, amountGp) or mass or undo)
	assert(type(name) == "string")
	...
	amountEp, amountGp = AddEPGP(main or name, amountEp, amountGp)
	if amountGp then
		callbacks:Fire("GPAward", name, reason, amountGp, mass, undo)
	end
	if amountEp then
		callbacks:Fire("EPAward", name, reason, amountEp, mass, undo)
	end
