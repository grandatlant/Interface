...
local function EPGPSideFrameGPDropDown_Initialize(dropDown)
  ...
  local arr = { 
				...
				{"Синдра: Бомба", 500 },
				{"Синдра: Дыхание", 500},
				{"Синдра: Обжигающий холод", 500},
				{"Синдра: Полученный урон", 500},
				{"Синдра: Взрыв в рейде", 500},
				{"----- Профессор ----", 500 },
				{"Проф: Пельмени", 500},
				{"Проф: Бомбы", 500},
				{"Проф: Слизень", 500},
				...
				{"Халион: Лезвие", 500},
				{"Халион: Диспел", 500},
				{"Халион: Не вынес метку", 500},
				{"------------------", 500 },
				{"Срыв агро", 500},
				{"Танкование", 500},
				{"Хреновый дпс/хпс", 500},
				{"Другое", 500}
				};
  
  for i=1,#arr do
    local info = UIDropDownMenu_CreateInfo()
    local itemLink = arr[i][1];
    info.text = itemLink
    info.arg1 = itemLink
    info.func = function(self, arg1)
                  UIDropDownMenu_SetSelectedID(dropDown, self:GetID())
                  parent.editBox:SetText(arr[i][2])
...
local function CreateEPGPSideFrame(self)
  ...
  f:SetScript(
	...
	local button1 = CreateFrame("Button", nil, f, "StaticPopupButtonTemplate")
	button1:SetPoint("BOTTOM", f, "BOTTOM", 0, 15)
	button1:SetText("Сбросить 30%");
	button1:SetScript(
    "OnClick",
    function(self)
		local ep, gp = EPGP:GetEPGP(f.name);
		local minusEp = ep * -0.3;
		local minusGp = gp * -0.3;
		
		local basegp = EPGP:GetBaseGP();
		local tmpGp = gp + minusGp;
		if tmpGp <= basegp then
			local tmpEp = ep + minusEp;
			
			local scale = basegp / tmpGp;
			
			minusEp = math.floor(tmpEp * scale - ep);
			minusGp = tmpGp * scale - gp;
			
			if(ep/gp < 500) then
				local link = "Сброс EP & GP";
				EPGP:IncEPGPBy(f.name, link, math.floor(minusEp), math.floor(minusGp));
			end
			
			--SendChatMessage("Сброс для "..f.name.." EP:"..ep.." GP:"..gp.." Minus EP:"..minusEp.." Minus GP:"..minusGp, 'GUILD', nil, 0 );
			...
			if ep/gp >= 1000 then
				...
				for i=1,members do
					if(GetGuildRosterInfo(i) == f.name) then
						GuildRosterSetOfficerNote(i, ep..","..(gp-basegp).." "..day.."."..month.."."..year);
						SendChatMessage("Заморожен <"..f.name.."> Дата: "..day.."."..month.."."..year, 'GUILD', nil, 0 );
						break;
					end
				end
			end
		else
			local link = "Сброс 30% EP & GP";
			EPGP:IncEPGPBy(f.name, link, math.floor(minusEp), math.floor(minusGp));
		end
    end)
end
