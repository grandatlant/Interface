...
function InterruptBar_Command(cmd)
  ...
  if cb then
  	...
  	ChatFrame1:AddMessage("InterruptBar Настройки | /ib <настройка>",0,1,0)  	
  	ChatFrame1:AddMessage("Масштаб -- scale <число> | значение: " .. InterruptBarDB.scale,0,1,0)
  	ChatFrame1:AddMessage("Спрятать/показать-- hidden  | значение: " .. tostring(InterruptBarDB.hidden),0,1,0)
  	ChatFrame1:AddMessage("Заблокировать -- lock  | значние: " .. tostring(InterruptBarDB.lock),0,1,0)
  	ChatFrame1:AddMessage("Тест -- test ",0,1,0)
  	ChatFrame1:AddMessage("Сброс -- reset ",0,1,0)
  end
end
...
local function InterruptBar_OnLoad(self)
	...
	ChatFrame1:AddMessage("Interrupt Bar by Kollektiv(перевод http://getaddon.com). Введите /ib для настроек.",0,1,0)
end
